...
class CronJobsPage(BasePage):
    ...
    def _select_option(self, field_id: str, value: str) -> None:
        ...
        current_value = select.first.locator(
            ".qwenpaw-select-selection-item, .ant-select-selection-item"
        )
        ...
        selector = select.first.locator(
            ".qwenpaw-select-selector, .ant-select-selector"
        )
        if selector.count() > 0:
        ...
        option = self.page.locator(
            f'.qwenpaw-select-item-option-content:has-text("{value}")'
        ).first
    ...
    def get_job_row(self, job_name: str) -> Locator:
        ...
        return self.page.locator(
            f"tr:not([aria-hidden='true']):not(.qwenpaw-table-placeholder):not(.qwenpaw-table-measure-row):has-text('{job_name}')"
        )
    ...
    def job_exists(self, job_name: str) -> bool:
        ...
        pagination_items = self.page.locator(
            ".qwenpaw-pagination-item:not(.qwenpaw-pagination-item-active)"
        ).all()
    ...
    def fill_job_form(
        ...
        if (
            request_textarea.count() > 0
            and request_textarea.first.is_visible()
        ):
    ...
    def save_job(self) -> "CronJobsPage":
        ...
        drawer = self.page.locator(
            ".qwenpaw-drawer:visible, .ant-drawer:visible"
        )
        if drawer.count() > 0:
            close_btn = self.page.locator(
                ".qwenpaw-drawer-close, .ant-drawer-close"
            )
    ...
    def click_edit_job(self, job_name: str) -> "CronJobsPage":
        ...
        if edit_btn.count() == 0:
            ...
            edit_btn = self.page.locator(
                '.ant-dropdown .ant-dropdown-menu-item:has-text("Edit")'
            )
        edit_btn.click()
        expect(self.page.locator(self.DRAWER).first).to_be_visible()
        return self
    ...
    def delete_job(
        self, job_name: str, confirm: bool = True
    ) -> "CronJobsPage":
        """Delete a job."""
        row = self.get_job_row(job_name)
        ...
        if confirm:
            ...
            confirm_btn = self.page.locator(
                '.ant-modal .ant-btn-danger:has-text("OK"), .qwenpaw-modal .qwenpaw-btn-danger:has-text("OK"), .ant-modal button:has-text("确定"), .qwenpaw-modal button:has-text("确定"), button:has-text("确认")'
            )
    ...
    def enable_job(self, job_name: str) -> "CronJobsPage":
        ...
        row = self.get_job_row(job_name)
        toggle = row.locator(self.ENABLE_TOGGLE)
        is_enabled = toggle.first.evaluate(
            "el => el.classList.contains('ant-switch-checked') || el.classList.contains('qwenpaw-switch-checked')"
        )
        if not is_enabled:
            toggle.click()
        return self
    ...
    def execute_job_now(self, job_name: str) -> "CronJobsPage":
        ...
        if execute_btn.count() == 0:
            # Try clicking the action menu
            row.locator(".ant-btn:has(svg)").first.click()
            execute_btn = self.page.locator(
                '.ant-dropdown .ant-dropdown-menu-item:has-text("Execute")'
            )
        execute_btn.click()
        return self
    ...
    def assert_job_not_exists(self, job_name: str) -> "CronJobsPage":
        ...
        assert not self.job_exists(
            job_name
        ), f"Job '{job_name}' should not exist"
        return self
    ...
    def assert_job_enabled(self, job_name: str) -> "CronJobsPage":
        ...
        if toggle.count() > 0:
            is_enabled = toggle.first.evaluate(
                "el => el.classList.contains('ant-switch-checked') || el.classList.contains('qwenpaw-switch-checked') || el.getAttribute('aria-checked') === 'true'"
            )
            ...
            assert (
                "启用" in row_text
                or "enabled" in row_text.lower()
                or "是" in row_text
            ), f"Job '{job_name}' should be enabled; row text: {row_text[:100]}"
        return self
    ...
    def assert_job_disabled(self, job_name: str) -> "CronJobsPage":
        ...
        if toggle.count() > 0:
            ...
            assert (
                "禁用" in row_text
                or "disabled" in row_text.lower()
                or "否" in row_text
            ), f"Job '{job_name}' should be disabled; row text: {row_text[:100]}"
        return self
