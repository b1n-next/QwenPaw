...
class TestSecurityToolGuardAndTabSwitch:
    ...
    def test_tool_guard_toggle_and_tab_switch(
        ...
        try:
            ...
            logger.warning(
                "Breadcrumb verification skipped (CN/EN did not match)"
            )
        ...
        tool_guard_switch = active_panel.locator(
        ...
        logger.info(
            f"Tool Guard switch initial state: aria-checked={initial_checked}"
        )
        ...
        file_guard_switch = file_guard_panel.locator(
            'button.qwenpaw-switch[role="switch"]'
        ).first
        expect(file_guard_switch).to_be_visible(timeout=5000)
        file_guard_checked = file_guard_switch.get_attribute("aria-checked")
        logger.info(
            f"File Guard switch state: aria-checked={file_guard_checked}"
        )

        # Step 9: Verify the File Guard path input
        log_test_step("9. Verify the File Guard path input")
        path_input = file_guard_panel.locator(
            'input[placeholder*="文件或目录路径"], input[placeholder*="file or directory"], input[placeholder*="File or directory"], input[placeholder*="path"]'
        ).first
        ...
        logger.info(
            f"Test {test_name} passed - Tool Guard switch and tab switching work"
        )
...
class TestSecurityFileGuardPathAndToolSelect:
    ...
    def test_file_guard_path_add_and_tool_select(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        add_button = file_guard_panel.locator(
        ...
        assert (
            filled_value == "~/.ssh/"
        ), f"Path was not filled correctly: {filled_value}"
        ...
        protected_tools_select = tool_guard_panel.locator(
            ".qwenpaw-select"
        ).first
        expect(protected_tools_select).to_be_visible(timeout=5000)
        # Click the selector inside the Select to trigger the dropdown
        select_selector = protected_tools_select.locator(
        ...
        if dropdown.count() > 0 and dropdown.is_visible():
            ...
            logger.info(
                f"Protected-tools dropdown options: {len(options)}, first: {first_option_text}"
            )
        else:
            # If the dropdown is not visible, verify that the Select at least exists and is interactive
            logger.info(
                "Protected-tools Select component exists and is interactive"
            )
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - File Guard path add and Tool Guard dropdown interaction work"
        )
...
class TestSecurityConfigSaveAndPersist:
    ...
    def test_security_config_save_and_persist(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        tool_guard_switch = tool_guard_panel.locator(
            'button.qwenpaw-switch[role="switch"]'
        ).first
        expect(tool_guard_switch).to_be_visible(timeout=5000)
        ...
        try:
            ...
            ), f"Switch state did not flip: {initial_checked} -> {new_checked}"
            ...
            success_msg = page.locator(
                '.qwenpaw-message-success, .qwenpaw-message-notice-content:has-text("保存")'
            ).first
            ...
            tool_guard_tab_refreshed = page.locator(
                '[data-node-key="toolGuard"] .qwenpaw-tabs-tab-btn'
            ).first
            ...
            tool_guard_switch_refreshed = tool_guard_panel_refreshed.locator(
                'button.qwenpaw-switch[role="switch"]'
            ).first
            expect(tool_guard_switch_refreshed).to_be_visible(timeout=5000)

            persisted_checked = tool_guard_switch_refreshed.get_attribute(
                "aria-checked"
            )
            assert (
                persisted_checked == new_checked
            ...
            try:
                tool_guard_switch_refreshed = (
                    page.locator(".qwenpaw-tabs-tabpane-active")
                    .first.locator('button.qwenpaw-switch[role="switch"]')
                    .first
                )
                if tool_guard_switch_refreshed.is_visible():
                    current_state = tool_guard_switch_refreshed.get_attribute(
                        "aria-checked"
                    )
                    if current_state != initial_checked:
                        ...
                        restored_checked = (
                            tool_guard_switch_refreshed.get_attribute(
                                "aria-checked"
                            )
                        )
                        assert (
                        ...
                        logger.info(
                            f"Switch restored to initial state: {restored_checked}"
                        )

                        # Save again
                        save_btn_refreshed = page.locator(
                            'button.qwenpaw-btn-primary:has-text("保存"), button:has-text("保 存")'
                        ).first
                        ...
                        logger.info(
                            "Switch already at initial state; no restore needed"
                        )
        ...
        skill_scanner_tab = page.locator(
            '[data-node-key="skillScanner"] .qwenpaw-tabs-tab-btn'
        ).first
        ...
        if skill_scanner_tab.is_visible():
            ...
            skill_scanner_panel = page.locator(
                ".qwenpaw-tabs-tabpane-active"
            ).first
            ...
            assert (
                len(content_elements) > 0
            ), "Skill Scanner tab content is empty"
            logger.info(
                f"Skill Scanner tab content loaded; element count: {len(content_elements)}"
            )
        else:
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - Security config save and persistence verified"
        )
...
class TestSecurityRuleCrud:
    ...
    def test_security_rule_crud(
        ...
        try:
            ...
            add_rule_btn = tool_guard_panel.locator(
                'button:has-text("添加规则"), button:has-text("Add Rule")'
            ).first
            ...
            rule_row = tool_guard_panel.locator(
                f'tr:has-text("{rule_id}")'
            ).first
            ...
            assert (
                initial_switch_state != after_disable
            ...
            assert (
                after_enable == "true"
            ), f"Rule was not re-enabled: {after_enable}"
            ...
            edit_btn = rule_row.locator(
                'button:not([role="switch"]):has(svg.lucide-pencil), button:not([role="switch"]):has(svg.lucide-Pencil)'
            ).first
            if edit_btn.count() == 0:
                edit_btn = rule_row.locator(
                    'button:not([role="switch"]):has(svg)'
                ).first
            ...
            if rule_id:
                try:
                    cleanup_rule_row = tool_guard_panel.locator(
                        f'tr:has-text("{rule_id}")'
                    ).first
                    if cleanup_rule_row.count() > 0:
                        delete_btn = cleanup_rule_row.locator(
                            'button:not([role="switch"]):has(svg.lucide-trash-2), button:not([role="switch"]):has(svg.lucide-Trash2)'
                        ).first
                        ...
                        confirm_delete_btn = page.locator(
                            '.qwenpaw-modal-confirm button.qwenpaw-btn-primary, .qwenpaw-modal button:has-text("确认"), .qwenpaw-modal button:has-text("Delete")'
                        ).first
                    ...
                    logger.warning(
                        f"Cleanup failed: could not delete test rule '{rule_id}'"
                    )

            # Restore the Tool Guard initial state
            if tool_guard_switch and initial_checked is not None:
                try:
                    current_state = tool_guard_switch.get_attribute(
                        "aria-checked"
                    )
                    if current_state != initial_checked:
                        tool_guard_switch.click()
                    ...
                    logger.warning(
                        "Cleanup failed: could not restore Tool Guard state"
                    )
...
class TestSkillScannerModeSwitch:
    ...
    def test_skill_scanner_mode_switch(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        current_mode_selector = mode_select.locator(
            ".qwenpaw-select-selector"
        ).first
        ...
        try:
            ...
            block_option = page.locator(
                '.qwenpaw-select-item:has-text("block"), .qwenpaw-select-item:has-text("Block")'
            ).first
            if block_option.count() > 0:
                ...
                assert (
                    "block" in new_mode_text.lower()
                ), f"Failed to switch to block mode: current display '{new_mode_text}'"
                logger.info(
                    f"Switched to block mode, display: {new_mode_text}"
                )
            else:
            ...
            warn_option = page.locator(
                '.qwenpaw-select-item:has-text("warn"), .qwenpaw-select-item:has-text("Warn")'
            ).first
            ...
            off_option = page.locator(
                '.qwenpaw-select-item:has-text("off"), .qwenpaw-select-item:has-text("Off")'
            ).first
            ...
            timeout_input = skill_scanner_panel.locator(
                'input[type="number"], .qwenpaw-input-number input'
            ).first
            ...
            scan_alerts_tab = skill_scanner_panel.locator(
                '[data-node-key="scanAlerts"] .qwenpaw-tabs-tab-btn, .qwenpaw-tabs-tab-btn:has-text("扫描警报"), .qwenpaw-tabs-tab-btn:has-text("Scan Alerts")'
            ).first
            ...
            whitelist_tab = skill_scanner_panel.locator(
                '[data-node-key="whitelist"] .qwenpaw-tabs-tab-btn, .qwenpaw-tabs-tab-btn:has-text("白名单"), .qwenpaw-tabs-tab-btn:has-text("Whitelist")'
            ).first
            ...
            log_test_result(test_name, True, 0)
            logger.info(
                f"Test {test_name} passed - Skill Scanner mode switching works"
            )
        finally:
            ...
            if original_mode_text and current_mode_selector:
                try:
                    ...
                    restore_option = page.locator(
                        f'.qwenpaw-select-item:has-text("{original_mode_text}")'
                    ).first
                    if restore_option.count() > 0:
                        restore_option.click()
                        page.wait_for_timeout(1000)
                        logger.info(
                            f"Restored original mode: {original_mode_text}"
                        )
                    else:
                        logger.warning(
                            f"Original mode option '{original_mode_text}' not found, cannot restore"
                        )
                except Exception:
                    logger.warning(
                        f"Failed to restore original mode '{original_mode_text}'"
                    )
...
class TestDeniedToolsConfig:
    ...
    def test_denied_tools_config(
        ...
        if denied_tools_select.count() == 0:
            ...
            denied_label = page.locator(
                ':text("Denied Tools"), :text("拒绝工具")'
            ).first
            if denied_label.count() > 0:
                denied_tools_select = denied_label.locator(
                    'xpath=ancestor::div[contains(@class, "form-item")]//div[contains(@class, "select")]'
                ).first
        ...
        if denied_tools_select.count() > 0:
            ...
            if len(options) > 0:
                ...
                logger.info(
                    "Dropdown options are empty, entering a custom tool name"
                )
            ...
            logger.info(
                "Denied tools list Select not found; verify the page has related form fields"
            )
            form_items = active_panel.locator(".qwenpaw-form-item").all()
            assert (
                len(form_items) >= 2
            ), f"Tool Guard form fields insufficient: {len(form_items)}"
...
class TestRulePreview:
    ...
    def test_rule_preview(self, page: Page, request: pytest.FixtureRequest):
        ...
        if rule_table.count() > 0:
            ...
            # Count rule rows
            rule_rows = active_panel.locator(
                "table tbody tr, .qwenpaw-table-row"
            ).all()
            ...
            if len(preview_btns) > 0:
                ...
                if preview_modal.count() > 0:
                    ...
                    logger.info(
                        f"Preview modal opened, content length: {len(modal_content)}"
                    )

                    # Close the modal
                    close_btn = preview_modal.locator(
                        '.qwenpaw-modal-close, button:has-text("Close"), button:has-text("关闭"), button:has-text("OK")'
                    ).first
                    ...
                    logger.info(
                        "Preview may be shown in another form (Drawer or inline)"
                    )
            else:
                logger.info(
                    "Standalone preview button not found; verify a rule row can be clicked to view details"
                )
                if len(rule_rows) > 0:
                    rule_rows[0].click()
                    page.wait_for_timeout(1000)
            ...
            logger.info(
                "Rules table not found; verify the page has rule-related content"
            )
            rule_content = active_panel.locator(
                ':text("rule"), :text("规则"), :text("Rule")'
            ).all()
...
class TestSecurityBatchRuleToggle:
    ...
    def test_security_batch_rule_toggle(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        if len(rule_switches) >= 2:
            ...
            for idx in range(1, len(rule_switches)):
                ...
                is_disabled = switch.get_attribute(
                    "disabled"
                ) is not None or "disabled" in (
                    switch.get_attribute("class") or ""
                )
                if not is_disabled:
            ...
            if target_switch is not None:
                ...
                logger.info(
                    f"Rule switch toggled: {original_state} -> {new_state}"
                )
                # Restore
                target_switch.click()
                page.wait_for_timeout(500)
                ...
                logger.info(
                    "All rule switches are disabled, skipping toggle test"
                )
        else:
