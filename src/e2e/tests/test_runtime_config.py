...
class TestReActAgentConfig:
    ...
    def test_react_agent_language_and_timezone(
        ...
        react_tab = page.locator(
            '[data-node-key="reactAgent"] .qwenpaw-tabs-tab-btn'
        ).first
        ...
        language_label = active_panel.locator(
            'label:has-text("Agent Language"), label:has-text("Language")'
        ).first
        try:
            ...
            logger.warning(
                "Agent language label not found, skipping verification"
            )
        ...
        current_value = language_select.locator(
        ...
        if dropdown.is_visible():
            ...
            assert (
                len(options) >= 2
            ), f"Insufficient language options: {len(options)}"
            assert (
                "English" in option_texts
            ), f"Missing English option: {option_texts}"
            logger.info(
                "Language dropdown opens correctly and options verified"
            )
            ...
            page.keyboard.press("Escape")
            logger.warning(
                "Language dropdown options did not open, skipping verification"
            )

        # Step 6: Verify the user timezone dropdown
        log_test_step("6. Verify the user timezone dropdown")
        timezone_label = active_panel.locator(
            'label:has-text("User Timezone"), label:has-text("Timezone")'
        ).first
        try:
            ...
            logger.warning(
                "User timezone label not found, skipping verification"
            )
        ...
        assert (
            len(selects) >= 2
        ), f"Insufficient dropdowns (expected >= 2): {len(selects)}"
        timezone_select = selects[1]
        timezone_value = timezone_select.locator(
            ".qwenpaw-select-selection-item"
        ).first.inner_text()
        ...
        logger.info(
            f"Test {test_name} passed - ReAct agent language and timezone config OK"
        )
...
class TestAgentConfigTabSwitch:
    ...
    def test_agent_config_tab_switch(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        for key in tab_keys:
            tab_btn = page.locator(
                f'[data-node-key="{key}"] .qwenpaw-tabs-tab-btn'
            ).first
        ...
        llm_retry_tab = page.locator(
            '[data-node-key="llmRetry"] .qwenpaw-tabs-tab-btn'
        ).first
        llm_retry_tab.click()
        ...
        retry_switches = retry_panel.locator(
            'button.qwenpaw-switch[role="switch"]'
        ).all()
        retry_inputs = retry_panel.locator(
            ".qwenpaw-input, .qwenpaw-input-number, .qwenpaw-select"
        ).all()
        assert (
            len(retry_switches) + len(retry_inputs) >= 1
        ), "No config items in LLM auto-retry tab"
        logger.info(
            f"LLM auto-retry tab - switches: {len(retry_switches)}, inputs: {len(retry_inputs)}"
        )

        # Step 4: Switch to the LLM rate limiter tab
        log_test_step("4. Switch to the LLM rate limiter tab")
        rate_limiter_tab = page.locator(
        ...
        rate_switches = rate_panel.locator(
            'button.qwenpaw-switch[role="switch"]'
        ).all()
        rate_inputs = rate_panel.locator(
            ".qwenpaw-input, .qwenpaw-input-number, .qwenpaw-select"
        ).all()
        assert (
            len(rate_switches) + len(rate_inputs) >= 1
        ), "No config items in LLM rate limiter tab"
        logger.info(
            f"LLM rate limiter tab - switches: {len(rate_switches)}, inputs: {len(rate_inputs)}"
        )
        ...
        context_inputs = context_panel.locator(
            ".qwenpaw-input, .qwenpaw-input-number, .qwenpaw-select"
        ).all()
        assert (
            len(context_inputs) >= 1
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - tab switching and content display OK"
        )
...
class TestAgentConfigSaveAndReset:
    ...
    def test_config_save_and_reset(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        ), f"Switch did not flip: {initial_checked} -> {new_checked}"
        ...
        success_msg = page.locator(
            '.qwenpaw-message-success, .qwenpaw-message-notice-content:has-text("Save")'
        ).first
        try:
        ...
        enable_switch_refreshed = context_panel_refreshed.locator(
            ".qwenpaw-switch"
        ).first
        expect(enable_switch_refreshed).to_be_visible(timeout=5000)

        persisted_checked = enable_switch_refreshed.get_attribute(
            "aria-checked"
        )
        assert (
        ...
        restored_checked = enable_switch_refreshed.get_attribute(
            "aria-checked"
        )
        assert (
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - config modification, save and persistence OK"
        )
...
class TestLlmRetryConfig:
    ...
    def test_llm_retry_config(self, page: Page):
        ...
        title_text = card_title.inner_text()
        assert (
            "LLM" in title_text or "Retry" in title_text
        ...
        logger.info(
            f"Original values: max_retries={original_max_retries}, base={original_backoff_base}, cap={original_backoff_cap}"
        )
        ...
        try:
            ...
            runtime_config_page.switch_to_llm_retry_tab()
            active_panel = page.locator(".qwenpaw-tabs-tabpane-active").first
            persisted_value = active_panel.locator(
                "#llm_max_retries"
            ).first.input_value()
            assert (
                persisted_value == "5"
            ), f"After reload, max_retries should be '5', got '{persisted_value}'"
            ...
            try:
                ...
                active_panel.locator("#llm_max_retries").first.fill(
                    original_max_retries
                )
                active_panel.locator("#llm_backoff_base").first.fill(
                    original_backoff_base
                )
                active_panel.locator("#llm_backoff_cap").first.fill(
                    original_backoff_cap
                )
                runtime_config_page.click_save()
                runtime_config_page.assert_config_saved()
...
class TestLlmRateLimiterConfig:
    ...
    def test_llm_rate_limiter_config(self, page: Page):
        ...
        title_text = card_title.inner_text()
        assert (
            "LLM" in title_text or "Rate" in title_text
        ), f"Card title does not contain expected keywords: {title_text}"
        ...
        # Record the original values
        log_test_step("Record original config values")
        max_concurrent_input = active_panel.locator(
            "#llm_max_concurrent"
        ).first
        ...
        logger.info(
            f"Original values: concurrent={original_concurrent}, qpm={original_qpm}, pause={original_pause}"
        )
        ...
        try:
            ...
            acquire_timeout_input = active_panel.locator(
                "#llm_acquire_timeout"
            ).first
            ...
            runtime_config_page.switch_to_llm_rate_limiter_tab()
            active_panel = page.locator(".qwenpaw-tabs-tabpane-active").first
            persisted_concurrent = active_panel.locator(
                "#llm_max_concurrent"
            ).first.input_value()
            assert (
                persisted_concurrent == "10"
            ), f"After reload, max_concurrent should be '10', got '{persisted_concurrent}'"
            ...
            try:
                ...
                active_panel.locator("#llm_max_concurrent").first.fill(
                    original_concurrent
                )
                active_panel.locator("#llm_max_qpm").first.fill(original_qpm)
                active_panel.locator("#llm_rate_limit_pause").first.fill(
                    original_pause
                )
                runtime_config_page.click_save()
                runtime_config_page.assert_config_saved()
...
class TestToolResultCompactConfig:
    ...
    def test_tool_result_compact_config(self, page: Page):
        ...
        log_test_step(
            "Switch to the context management tab (tool-result compaction was merged here)"
        )
        ...
        assert any(
            kw in panel_text for kw in ["Context", "Compact"]
        ...
        inputs = active_panel.locator(
            ".qwenpaw-input, .qwenpaw-input-number, .qwenpaw-select"
        ).all()
        assert (
            len(inputs) >= 1
        ), f"No config items found on context management panel; found {len(inputs)}"
        logger.info(
            f"Found {len(inputs)} config items on context management panel"
        )
...
class TestEmbeddingConfig:
    ...
    def test_embedding_config(self, page: Page):
        ...
        log_test_step(
            "Switch to the long-term memory tab (embedding config was merged here)"
        )
        ...
        panel_text = active_panel.inner_text()
        assert any(
            kw in panel_text for kw in ["Memory", "Embedding"]
        ...
        assert (
            len(switches) >= 1
        ), f"No switches found on long-term memory panel; found {len(switches)}"
        logger.info(
            f"Found {len(switches)} switches on long-term memory panel"
        )
...
class TestContextCompactConfig:
    ...
    def test_context_compact_config(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        title_text = card_title.inner_text()
        assert (
            "Context" in title_text or "Compact" in title_text
        ), f"Card title does not contain expected keywords: {title_text}"
        ...
        logger.info(
            f"Found config items: switches={len(switches)}, sliders={len(sliders)}, inputs={len(inputs)}"
        )
        ...
        if len(switches) >= 1:
            ...
            logger.info(
                "Context management tab has no switch controls; skipping toggle test"
            )
...
class TestConfigDynamicLinkage:
    ...
    def test_config_dynamic_linkage(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        for i in range(min(3, len(tabs))):
            ...
            logger.info(
                f"Tab {i+1} panel content length: {len(panel_content)}"
            )
...
class TestMemorySummaryConfig:
    ...
    def test_memory_summary_config(
        self, page: Page, request: pytest.FixtureRequest
    ):
