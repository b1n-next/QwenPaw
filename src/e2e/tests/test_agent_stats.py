...
class TestAgentStatsPageDisplay:
    ...
    def test_agent_stats_page_load_and_cards(
        ...
        try:
            ...
            if breadcrumb.is_visible(timeout=3000):
                ...
                assert (
                    "Agent Stats" in breadcrumb_text
                    or "Statistics" in breadcrumb_text
                    or "统计" in breadcrumb_text
                    or "Stats" in breadcrumb_text
                ), "Breadcrumb should contain Agent Stats/Statistics"
                logger.info("Breadcrumb validation passed")
            else:
                logger.warning(
                    "Breadcrumb element not found, skipping validation"
                )
            ...
            has_cards_or_empty = len(cards) > 0 or empty.is_visible(
                timeout=3000
            )
            assert (
                has_cards_or_empty
...
class TestAgentStatsDatePicker:
    ...
    def test_date_range_picker_interaction(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        try:
            ...
            date_cells = panel.locator(
                '.qwenpaw-picker-cell, td[class*="cell"]'
            )
            ...
            if cell_count > 0:
                ...
                has_content = (
                    len(panel_text.strip()) > 0
                    or len(panel_html.strip()) > 100
                )
                assert (
                    has_content
                ), "Calendar panel should contain date content"
                logger.info(
                    f"Calendar panel content snippet: {panel_text[:100]}"
                )
...
class TestAgentStatsCharts:
    ...
    def test_chart_area_display(
        ...
        try:
            ...
            svg_charts = page.locator(
                "svg[class*='chart'], svg[class*='g2']"
            ).all()
            ...
            logger.info(
                f"Canvas elements: {len(canvas_elements)}, SVG charts: {len(svg_charts)}, "
            ...
            if total_charts > 0:
                ...
                logger.info(
                    "Chart area not displayed when no data (empty state)"
                )
...
class TestAgentStatsChannelDistribution:
    ...
    def test_channel_distribution_display(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        try:
            ...
            has_channel_section = (
                "Channel Distribution" in page_text
                or "渠道分布" in page_text
            ...
            empty = page.locator(
                ".qwenpaw-empty, .ant-empty, [class*='empty']"
            ).first
            assert has_channel_section or (
                empty.count() > 0 and empty.is_visible(timeout=3000)
            ...
            if has_channel_section:
                ...
                has_chart_element = (
                    len(pie_containers) > 0 or len(canvas_in_page) > 0
                )
                if has_chart_element:
                    logger.info(
                        f"Found chart elements (pie containers: {len(pie_containers)}, canvas: {len(canvas_in_page)})"
                    )
                else:
                    logger.info(
                        "Channel distribution section present but no chart rendered (possibly no data)"
                    )
            else:
                logger.info(
                    "Page displays empty state (no channel distribution data)"
                )
            ...
            found_channels = [
                ch for ch in channel_names if ch.lower() in page_text.lower()
            ]
            if found_channels:
...
class TestAgentStatsDateFilter:
    ...
    def test_date_filter_refreshes_data(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        try:
            ...
            assert len(cards_after) > 0 or empty_state.is_visible(
                timeout=3000
            ), "After date filter, should still have cards or display empty state"
            logger.info(
                f"Date filter refresh validation passed (cards: {len(cards_after)})"
            )
...
class TestAgentStatsCardTooltip:
    ...
    def test_card_tooltip_display(
        ...
        try:
            ...
            if len(cards) == 0:
                logger.info(
                    "No summary cards found, skipping tooltip validation"
                )
                log_test_result(test_name, True, 0)
                return
            ...
            hover_target = (
                info_icon if info_icon.is_visible(timeout=3000) else first_card
            )
            hover_target.hover()
            page.wait_for_timeout(500)
            ...
            if tooltip.is_visible(timeout=3000):
                ...
                logger.info(
                    "Tooltip not triggered (possibly no tip info), hover interaction OK"
                )
...
class TestAgentStatsEmptyAndLoading:
    ...
    def test_empty_and_loading_states(
        ...
        try:
            ...
            spin = page.locator(
                ".qwenpaw-spin, [class*='loading'], [class*='spin']"
            ).first
            if spin.is_visible(timeout=3000):
                ...
                logger.info(
                    "Loading state not captured (possibly loaded too fast)"
                )
...
class TestAgentStatsRefresh:
    ...
    def test_page_refresh_data_persistence(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        try:
            ...
            logger.info(
                f"Before refresh: cards={card_count_before}, canvas={canvas_count_before}"
            )
            ...
            logger.info(
                f"After refresh: cards={card_count_after}, canvas={canvas_count_after}"
            )
