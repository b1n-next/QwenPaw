...
class TestVoiceConfigDisplay:
    ...
    def test_voice_config_display(
        ...
        has_radio = radio_group.count() > 0 and radio_group.is_visible(
            timeout=3000
        )
        has_switch = voice_switch.count() > 0 and voice_switch.is_visible(
            timeout=2000
        )
        all_controls = page.locator(
            ".qwenpaw-radio-wrapper, .qwenpaw-switch, .qwenpaw-select, input"
        ).all()
        assert (
            len(all_controls) > 0
        ), "Voice page should have at least one interactable config control"
        logger.info(
            f"Found {len(all_controls)} config controls (Radio={'yes' if has_radio else 'no'}, Switch={'yes' if has_switch else 'no'})"
        )

        # Step 5: Verify config form
        log_test_step("5. Verify config form fields")
        form_fields = page.locator(
            ".qwenpaw-form-item, .qwenpaw-radio-wrapper, input, .qwenpaw-select, textarea"
        ).all()
        assert (
            len(form_fields) > 0
        ...
        if has_radio:
            ...
            assert (
                len(radio_items) >= 2
            ), f"Should have at least 2 Radio options, actual {len(radio_items)}"
        ...
        save_btn = page.locator(
            'button:has-text("保存"), button:has-text("Save"), button.qwenpaw-btn-primary'
        ).first
        ...
        log_test_result(
            test_name,
            "PASS",
            "Voice transcription config display and controls validation passed",
        )
...
class TestVoiceToggle:
    ...
    def test_voice_service_toggle(
        ...
        has_switch = voice_toggle.count() > 0 and voice_toggle.is_visible(
            timeout=3000
        )
        ...
        if has_radio:
            logger.info(
                "Found Radio.Group config control (provider type selection)"
            )
            ...
            assert (
                len(radio_items) >= 2
            ), f"Should have at least 2 config options, actual {len(radio_items)}"
            logger.info(f"Found {len(radio_items)} config options")

            # Get the currently checked option
            checked_radio = page.locator(
                ".qwenpaw-radio-wrapper-checked, .qwenpaw-radio-wrapper.qwenpaw-radio-wrapper-checked"
            ).first
            initial_text = ""
            ...
            new_checked = page.locator(
                ".qwenpaw-radio-wrapper-checked, .qwenpaw-radio-wrapper.qwenpaw-radio-wrapper-checked"
            ).first
            if new_checked.count() > 0:
                new_checked_text = new_checked.text_content() or ""
                assert (
                    new_checked_text != initial_text or initial_text == ""
            ...
            initial_state = (
                "checked" in toggle_class if toggle_class else False
            )
            logger.info(
                f"Initial state: {'enabled' if initial_state else 'disabled'}"
            )
            ...
            new_state = (
                "checked" in new_toggle_class if new_toggle_class else False
            )
            ...
            assert (
                len(visible_controls) > 0
            ), "No voice service config control found (Radio/Switch/Card/Select/Input)"
            logger.info(
                f"Found {len(visible_controls)} config controls; voice page uses non-standard layout"
            )
            ...
            if voice_select.count() > 0 and voice_select.is_visible():
                ...
                dropdown_items = page.locator(
                    ".qwenpaw-select-item, .ant-select-item, [class*=select-item]"
                ).all()
                logger.info(
                    f"Select control is interactable, found {len(dropdown_items)} dropdown options"
                )
                page.keyboard.press("Escape")
                page.wait_for_timeout(500)

        log_test_result(
            test_name, "PASS", "Voice service config toggle validation passed"
        )
...
class TestVoiceServiceConfig:
    ...
    def test_twilio_config_form(
        ...
        twilio_section = page.locator(
            '[class*=twilio], .qwenpaw-card:has-text("Twilio"), .qwenpaw-collapse:has-text("Twilio")'
        ).first
        page_content = page.locator("body").inner_text()
        has_twilio_content = any(
            keyword in page_content
            for keyword in [
                "Twilio",
                "Account SID",
        ...
        assert (
            has_twilio_content
            or twilio_section.count() > 0
            or has_voice_content
        ...
        if has_twilio_content:
            ...
            logger.info(
                "Page lacks Twilio content but has generic voice config content"
            )

        # Step 3: Verify config fields and test input
        log_test_step("3. Verify config fields and test input")
        all_inputs = page.locator(
            'input[type="text"], input[type="password"], .qwenpaw-input input, input'
        ).all()
        ...
        webhook_url = (
            page.locator(
                '[class*=webhook], .qwenpaw-paragraph:has-text("/voice/")'
            )
            .or_(page.get_by_text("Webhook", exact=False))
            .first
        )
...
class TestVoiceModeSwitch:
    ...
    def test_voice_mode_switch(
        ...
        all_controls = page.locator(
            ".qwenpaw-switch, .qwenpaw-select, .ant-select, input, .qwenpaw-radio-group"
        ).all()
        assert (
            len(all_controls) > 0
        ), "Voice config page should have config controls"
        logger.info(
            f"Found {len(switches)} switch controls, total {len(all_controls)} config controls"
        )
