...
class TestChannelListAndFilter:
    ...
    def test_channel_list_filter_and_type(
        ...
        assert (
            "QwenPaw" in page_title or "Channels" in page_title
        ), f"Unexpected page title: {page_title}"

        log_test_step("2. Verify the filter buttons are visible")
        assert channels_page.page.locator(
            channels_page.FILTER_ALL_BTN
        ).first.is_visible(), "All filter button not shown"
        assert channels_page.page.locator(
            channels_page.FILTER_BUILTIN_BTN
        ).first.is_visible(), "Built-in filter button not shown"
        assert channels_page.page.locator(
            channels_page.FILTER_CUSTOM_BTN
        ).first.is_visible(), "Custom filter button not shown"
        ...
        for channel_name in builtin_channels:
            ...
            if card:
                assert channels_page.is_builtin_channel(
                    channel_name
        ...
        assert channels_page.verify_filter_result(
            "builtin"
        ), "Built-in filter results include non-built-in channels"
        ...
        if custom_count > 0:
            assert channels_page.verify_filter_result(
                "custom"
            ), "Custom filter results include non-custom channels"
        ...
        assert (
            restored_count == all_count
        ), f"Count mismatch after All filter restore: expected {all_count}, actual {restored_count}"

        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - channel list, filter, and type tags are all correct"
        )
...
class TestConsoleEditConfig:
    ...
    def test_console_edit_save_cancel(
        self, channels_page: ChannelsPage, request: pytest.FixtureRequest
    ):
        """
        ...
        assert (
            channel_name in title_first_line
            or channel_name_cn in title_first_line
        ...
        assert (
            bot_input.count() > 0 and bot_input.is_visible()
        ...
        log_test_step(
            "4. Record the original value, modify Bot Prefix and save"
        )
        ...
        try:
            ...
            if channels_page.has_form_validation_errors():
                pytest.fail(
                    f"Console channel should have no form validation errors, but errors were detected"
                )

            channels_page.page.wait_for_timeout(2000)
            drawer_still_open = channels_page.page.locator(
                ".qwenpaw-drawer-open, .ant-drawer-open"
            ).first
            if drawer_still_open.count() > 0 and drawer_still_open.is_visible(
                timeout=1000
            ):
                channels_page.close_drawer()
                channels_page.page.wait_for_timeout(1000)
            try:
                channels_page.page.wait_for_load_state(
            ...
            channels_page.page.wait_for_timeout(1000)
            saved_prefix = channels_page.page.locator(
                "#bot_prefix"
            ).input_value()
            assert (
                saved_prefix == test_prefix
            ...
            log_test_step(
                "7. Reopen the drawer and verify cancel did not take effect"
            )
            channels_page.click_channel_card(channel_name)
            channels_page.wait_for_drawer_open()
            channels_page.page.wait_for_timeout(1000)
            after_cancel_prefix = channels_page.page.locator(
                "#bot_prefix"
            ).input_value()
            assert (
                after_cancel_prefix == test_prefix
            ...
            log_test_result(test_name, True, 0)
            logger.info(
                f"Test {test_name} passed - Console edit save and cancel work correctly"
            )
        finally:
            ...
            try:
                ...
                channels_page.wait_for_drawer_open()
                channels_page.page.wait_for_timeout(500)
                current_prefix = channels_page.page.locator(
                    "#bot_prefix"
                ).input_value()
                if current_prefix != original_prefix:
                    ...
                    channels_page.save_channel_config()
                    logger.info(
                        f"Restored original Bot Prefix: '{original_prefix}'"
                    )
                channels_page.close_drawer()
...
class TestDiscordEnableDisable:
    ...
    def test_discord_enable_disable_ui(
        self, channels_page: ChannelsPage, request: pytest.FixtureRequest
    ):
        """
        ...
        drawer_title = channels_page.get_drawer_title()
        assert (
            channel_name in drawer_title
        ), f"Unexpected drawer title: {drawer_title}, expected to contain {channel_name}"
        logger.info(f"Drawer title: {drawer_title}")

        log_test_step("3. Read the current switch state")
        switch = channels_page.page.locator(
            ".qwenpaw-switch, .ant-switch"
        ).first
        ...
        if has_errors:
            logger.info(
                "Save failed; form validation errors detected (expected)"
            )
        else:
            logger.warning(
                "Save did not produce validation errors; Discord may already have defaults"
            )
        ...
        assert (
            after_reopen_checked == initial_checked
        ), f"Switch state did not revert: expected {initial_checked}, actual {after_reopen_checked}"
        logger.info(
            f"Switch state reverted: {after_reopen_checked} (not persisted)"
        )
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - Discord switch UI toggle works, not persisted"
        )
...
class TestMultipleChannelFormFields:
    ...
    def test_four_channels_form_fields(
        self, channels_page: ChannelsPage, request: pytest.FixtureRequest
    ):
        """
        ...
        for channel_name, expected_field_keywords in channels_to_check:
            log_test_step(
                f"2. Open the {channel_name} drawer and verify form fields"
            )
            channels_page.click_channel_card(channel_name)
            assert (
                channels_page.wait_for_drawer_open()
            ), f"{channel_name} edit drawer did not open"
            ...
            logger.info(
                f"{channel_name} found field keywords: {found_keywords}"
            )
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - four-channel form-field verification OK"
        )
...
class TestMattermostComboOperations:
    ...
    def test_mattermost_filter_edit_cancel(
        self, channels_page: ChannelsPage, request: pytest.FixtureRequest
    ):
        """
        ...
        assert (
            card is not None
        ), f"{channel_name} channel not found under the Built-in filter"
        channels_page.click_channel_card(channel_name)
        ...
        assert (
            after_cancel_prefix == original_prefix
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - Mattermost filter+edit+cancel combination works"
        )
...
class TestMessageFilterSwitches:
    ...
    def test_message_filter_switches(
        self, channels_page: ChannelsPage, request: pytest.FixtureRequest
    ):
        """
        ...
        log_test_step(
            "2. Iterate channels, looking for ones with Show Tool Messages / Show Thinking switches"
        )
        ...
        for channel_name in candidate_channels:
            ...
            log_test_step(
                f"3. Check whether channel {channel_name} has a message-filter switch"
            )
            channels_page.click_channel_card(channel_name)
            if not channels_page.wait_for_drawer_open():
                logger.warning(
                    f"Could not open {channel_name} drawer, skipping"
                )
                continue
            ...
            if not (has_tool_messages or has_thinking):
                logger.info(
                    f"Channel {channel_name} has no message-filter switch, closing drawer and continuing"
                )
                channels_page.close_drawer()
                channels_page.page.wait_for_timeout(500)
                continue
            ...
            if target_switch is not None:
                ...
                try:
                    ...
                    logger.info(
                        f"After toggle {switch_label} state: {new_state}"
                    )

                    assert (
                        initial_state != new_state
                    ), f"{switch_label} switch state did not change: {initial_state}"
                    logger.info(f"{switch_label} toggled successfully")
                finally:
                    # Whether the assertion passed or not, restore the original state
                    try:
                        current_state = target_switch.get_attribute(
                            "aria-checked"
                        )
                        if current_state != initial_state:
                            ...
                            logger.info(
                                f"Restored {switch_label} to initial state: {initial_state}"
                            )
                    except Exception as restore_err:
                        logger.warning(
                            f"Failed to restore switch state: {restore_err}"
                        )
        ...
        assert (
            len(found_switch_channels) > 0
        ), "No channel with a message-filter switch was found"
        logger.info(
            f"Channels with message-filter switches: {found_switch_channels}"
        )

        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - message-filter switches verified"
        )
...
class TestWeComFormFields:
    ...
    def test_wecom_form_fields(
        ...
        found_keywords = [
            ...
            if kw.lower() in drawer_content.lower()
        ]
...
class TestWeChatFormFields:
    ...
    def test_wechat_form_fields(
        ...
        found_unique = [
            kw
            for kw in wechat_unique_keywords
            if kw.lower() in drawer_content.lower()
        ]
...
class TestMQTTBotPrefix:
    ...
    def test_mqtt_bot_prefix(
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - MQTT Bot Prefix configuration verified"
        )
