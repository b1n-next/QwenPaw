    ...
    def test_auth_status_api(
        ...
        try:
            ...
            if response.ok:
                ...
                logger.info(
                    f"Auth status API returned {response.status} (auth may be disabled)"
                )

            log_test_result(
                test_name, "PASS", "Auth status API validation passed"
            )
    ...
    def test_login_page_accessible(
        ...
        try:
            ...
            if login_elements.is_visible(timeout=3000):
                ...
                logger.info(
                    "Login page may have auto-logged in or auth is disabled"
                )

            log_test_result(
                test_name, "PASS", "Login page accessibility validation passed"
            )
        except Exception as e:
            log_test_result(test_name, "FAIL", str(e))
            raise
...
class TestMultiUserManagement:
    ...
    def test_multi_user_management(
        self, page: Page, request: pytest.FixtureRequest, api_context
    ):
        ...
        has_form = (
            ...
            or register_btn.count() > 0
        )
        if has_form:
            logger.info("Login/register form validation passed")
        else:
            logger.info(
                "No login form found, may have auto-logged in or auth disabled"
            )
...
class TestLoginFormValidation:
    ...
    def test_login_empty_form_validation(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        if not submit_btn.is_visible(timeout=3000):
            logger.info(
                "No login form found (auth may be disabled), skipping form validation test"
            )
            log_test_result(test_name, True, 0)
            return
        ...
        if len(validation_errors) > 0:
            ...
            logger.info(
                f"Displayed {len(validation_errors)} required-field validation messages"
            )
        else:
            logger.info(
                "No form validation messages shown (may use other validation method)"
            )
    ...
    def test_login_partial_form_validation(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        if not submit_btn.is_visible(timeout=3000):
            logger.info(
                "No login form found (auth may be disabled), skipping test"
            )
            log_test_result(test_name, True, 0)
            return
