...
class TestSessionListFilterAndDetail:
    ...
    def test_session_list_filter_and_detail(
        ...
        assert (
            header_count >= 3
        ), f"Table should have at least 3 columns, got {header_count}"
        logger.info(f"Table column count: {header_count}")

        log_test_step("3. Verify filters and run a filter operation")
        userid_input = sessions_page.page.locator(
            sessions_page.FILTER_USER_ID_INPUT
        ).first
        ...
        if refreshed_count > 0:
            visible_rows = sessions_page.page.locator(
                "tbody tr:not([aria-hidden='true'])"
            )
            assert (
                visible_rows.count() > 0
            ...
            detail_panel = sessions_page.page.locator(
                '.qwenpaw-drawer, .qwenpaw-modal, [class*="detail"]'
            ).first
            if detail_panel.count() > 0 and detail_panel.is_visible(
                timeout=3000
            ):
                ...
                logger.info(
                    f"Session detail panel opened, content length: {len(detail_text)}"
                )
                sessions_page.page.keyboard.press("Escape")
                sessions_page.page.wait_for_timeout(500)
            else:
                logger.info(
                    "Clicking row did not open a detail panel (may use route navigation)"
                )
        ...
        logger.info(
            f"Test {test_name} passed - session list display, filter, sort and detail OK"
        )
...
class TestEditAndDeleteSession:
    ...
    def test_edit_and_delete_session(
        self,
        ...
        first_row = sessions_page.page.locator(
            sessions_page.SESSION_TABLE_ROW
        ).first
        ...
        logger.info(
            f"Batch-delete button appeared after selecting checkbox ({batch_btns.count()})"
        )
        ...
        log_test_result(test_name, True, 0)
        logger.info(
            f"Test {test_name} passed - edit, delete and batch-delete verified"
        )
...
class TestSessionEditAndSave:
    ...
    def test_session_edit_name_and_save(
        ...
        original_name_cell = first_row.locator("td").nth(
            1
        )  # Assume the name is in the 2nd column
        original_name = (
            original_name_cell.text_content().strip()
            if original_name_cell.count() > 0
            else ""
        )
        ...
        if not edit_btn.is_visible():
            ...
            fixed_row = sessions_page.page.locator(
        ...
        name_input = (
            sessions_page.page.locator(sessions_page.SESSION_DRAWER)
            .first.locator("input")
            .first
        )
        if name_input.count() > 0 and name_input.is_visible():
            ...
            name_input = sessions_page.page.locator(
        ...
        save_btn = sessions_page.page.locator(
            sessions_page.FORM_SUBMIT_BTN
        ).first
        if save_btn.count() == 0:
            save_btn = sessions_page.page.locator(
                'button:has-text("Save"), button.qwenpaw-btn-primary'
            ).first
        ...
        log_test_step(
            "8. Verify the new name appears in the list (hard assert)"
        )
        sessions_page.page.wait_for_timeout(1500)
        ...
        if original_name:
            ...
            if name_input.count() > 0:
                ...
                if save_btn.count() > 0:
                    save_btn.click()
                    expect(
                        sessions_page.page.locator(
                            sessions_page.SESSION_DRAWER
                        ).first
                    ).to_be_hidden(timeout=5000)
...
class TestSessionBatchDelete:
    ...
    def test_session_batch_delete(
        ...
        if session_count < 2:
            pytest.skip(
                f"Insufficient sessions; need at least 2, have {session_count}"
            )
        ...
        if len(row_checkboxes) < 2:
            pytest.skip(
                f"Not enough row checkboxes found; got {len(row_checkboxes)}"
            )
        ...
        modal_visible = False
        try:
            modal = sessions_page.page.locator(
                ".qwenpaw-modal, .ant-modal"
            ).first
            ...
            logger.warning(
                "No dialog detected, trying to match the confirm button directly"
            )
        ...
        if confirm_btn is not None:
            ...
            logger.warning(
                "Confirm-dialog button not found, trying to confirm via Enter"
            )
        ...
        logger.info(
            f"Session count went from {count_before} down to {count_after}"
        )
...
def ensure_session_data(page: Page):
    ...
    if needed == 0:
        logger.info(
            f"Already have {existing_count} sessions; no creation needed"
        )
        yield created_session_ids
        return

    logger.info(
        f"Currently {existing_count} sessions; need to create {needed} more"
    )
    ...
    logger.info(
        f"Starting cleanup of {len(created_session_ids)} test sessions"
    )
    try:
        ...
        for match in chat_list.get("matches", []):
            ...
            try:
                ...
                if delete_result.get("ok"):
                    ...
                    logger.warning(
                        f"  Delete failed: {sid} -> {delete_result}"
                    )
...
class TestSessionFilterByUseridAndChannel:
    ...
    def test_session_filter_by_userid_and_channel(
        self, page: Page, ensure_session_data
    ):
        ...
        assert (
            has_userid_input or has_channel_select
        ), "No filter controls found (UserID input or Channel selector)"
        logger.info(
            f"Filter controls: UserID input={'yes' if has_userid_input else 'no'}, Channel selector={'yes' if has_channel_select else 'no'}"
        )
        ...
        if has_userid_input:
            ...
            logger.info(
                f"After UserID filter: {filtered_count} sessions (initial: {initial_count})"
            )
            ...
            if filtered_count > 0:
                ...
                if len(first_filtered_cells) >= 2:
                    result_userid = (
                        first_filtered_cells[1].inner_text().strip()
                    )
                    assert (
                        test_userid in result_userid
                        or result_userid in test_userid
            ...
            assert (
                abs(restored_count - initial_count) <= 2
            ), f"Restored count is off: initial {initial_count}, restored {restored_count}"
            logger.info(
                f"After clearing filter, restored to {restored_count} sessions (initial {initial_count})"
            )
