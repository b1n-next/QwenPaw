...
class TestDebugPageDisplay:
    ...
    def test_debug_page_load_and_display(
        ...
        assert any(
            kw in alert_text for kw in debug_keywords
        ...
        log_content = page.locator(
            '[style*="monospace"], [style*="pre-wrap"]'
        ).first
        if log_content.is_visible(timeout=5000):
            content_text = log_content.inner_text()
            logger.info(
                f"Log content area visible, length: {len(content_text)}"
            )
        else:
...
class TestDebugLogControls:
    ...
    def test_debug_log_control_buttons(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        refresh_btn = page.locator(
            'button:has-text("Refresh"), ' 'button:has-text("刷新")',
        ).first
        ...
        if error_alert.is_visible(timeout=2000):
            logger.info(
                "Error alert appears after refresh (backend may be down)"
            )
        else:
        ...
        switches = page.locator(
            ".qwenpaw-card-extra .qwenpaw-switch, .qwenpaw-card-head .qwenpaw-switch"
        ).all()
        assert (
            len(switches) >= 2
        ), f"Should have at least 2 switches (auto-refresh + newest-first), actual: {len(switches)}"
        ...
        logger.info(
            f"Auto-refresh toggle changed: {initial_state} -> {new_state}"
        )
        ...
        logger.info(
            f"Newest-first toggle changed: {initial_order} -> {new_order}"
        )
...
class TestDebugLogLevelFilter:
    ...
    def test_debug_log_level_filter(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        for level in expected_levels:
            option = dropdown.locator(
                f'.qwenpaw-select-item:has-text("{level}")'
            ).first
            if option.is_visible(timeout=2000):
                logger.info(f"  Level option '{level}' exists")
            else:
                logger.info(
                    f"  Level option '{level}' not found (may be rendered as Tag)"
                )
        ...
        all_option = page.locator(
            ".qwenpaw-select-dropdown .qwenpaw-select-item"
        ).first
...
class TestDebugLogSearch:
    ...
    def test_debug_log_keyword_search(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        clear_btn = (
            ...
            .locator(".qwenpaw-input-clear-icon")
            .first
        )
    ...
    def test_debug_log_file_info(
        ...
        if updated_text.is_visible(timeout=3000):
            ...
            if updated_text_zh.is_visible(timeout=2000):
                ...
                logger.info(
                    "Updated timestamp not shown (logs not yet loaded)"
                )
