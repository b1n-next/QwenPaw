...
class TestTokenUsageDisplay:
    ...
    def test_token_usage_overview(
        ...
        try:
            ...
            if breadcrumb_settings.is_visible(timeout=3000):
                breadcrumb_current = page.locator(
                    'span[class*="breadcrumbCurrent"]:has-text("Token 消耗"), span[class*="breadcrumbCurrent"]:has-text("Token Usage")'
                ).first
            ...
            logger.info(
                f"Breadcrumb validation skipped (page structure may differ): {e}"
            )

        # Step 3: Verify page title
        log_test_step("3. Verify page title")
        page_title = page.locator(
            'h1:has-text("Token Usage"), h1:has-text("Token"), .qwenpaw-page-header:has-text("Token")'
        ).first
        ...
        if len(overview_cards) > 0:
            ...
            logger.info(
                "No overview cards found (page may use different layout or have no data)"
            )

        # Step 5: Check for data table or empty state
        log_test_step("5. Verify data table or empty state")
        table_area = page.locator(
            ".qwenpaw-table, .ant-table, table, [class*=dataTable]"
        ).first
        empty_state = page.locator(
            ".qwenpaw-empty, .ant-empty, [class*=empty], [class*=Empty]"
        ).first
        has_table = table_area.count() > 0 and table_area.is_visible(
            timeout=5000
        )
        ...
        if not has_table and not has_empty:
            main = page.locator(
                'main, [class*="content"], [class*="pageContent"]'
            ).first
            ...
            if any(kw in page_text for kw in empty_state_keywords):
                logger.info(
                    "Page shows text-based empty state (no table or empty component rendered)"
                )
                log_test_result(
                    test_name, "PASS", "Token usage page empty state validated"
                )
                return

        assert (
            has_table or has_empty
        ...
        if has_table:
            ...
            if len(visible_headers) > 0:
                header_texts = [
                    h.inner_text().strip() for h in visible_headers
                ]
            ...
            logger.info(
                "Empty state displayed correctly (no token usage data)"
            )

        log_test_result(
            test_name,
            "PASS",
            "Token usage overview display and interaction validation passed",
        )
...
class TestTokenUsageByModel:
    ...
    def test_token_usage_by_model(
        ...
        if len(tables) > 0:
            ...
            header_texts = [
                h.inner_text().strip() for h in headers if h.is_visible()
            ]
            logger.info(f"Table column headers: {header_texts}")
            assert len(header_texts) > 0, "Table has no column headers"
            logger.info(
                "By-model statistics table column headers validation passed"
            )

            log_test_step("Verify data rows or empty state")
            data_rows = first_table.locator(
                "tbody tr, .qwenpaw-table-row"
            ).all()
            empty_state = first_table.locator(
                '.qwenpaw-empty, :text("暂无"), :text("No data")'
            ).first

            assert (
                len(data_rows) > 0 or empty_state.count() > 0
            ), "By-model statistics table should have data rows or show empty state"
            if len(data_rows) > 0:
                logger.info(
                    f"By-model statistics table has {len(data_rows)} data rows"
                )
            else:
                logger.info("By-model statistics table shows empty state")
        else:
            logger.info(
                "No table found; verify the page has statistics-related content"
            )
            stat_content = page.locator(
                ':text("Model"), :text("模型"), :text("Token")'
            ).all()
            logger.info(
                f"Found {len(stat_content)} statistics-related elements"
            )
    ...
    def test_token_usage_by_date(
        ...
        if len(tables) >= 2:
            ...
            if has_date_column:
                ...
                logger.info(
                    f"No explicit date column found; headers: {header_texts}"
                )

            log_test_step("Verify data rows")
            data_rows = date_table.locator(
                "tbody tr, .qwenpaw-table-row"
            ).all()
            logger.info(
                f"By-date statistics table has {len(data_rows)} data rows"
            )
        elif len(tables) == 1:
            logger.info(
                "Only 1 table found; by-model and by-date may be combined"
            )
            # Verify table has Tab switching
            tabs = page.locator(
                ".qwenpaw-tabs-tab, .qwenpaw-segmented-item"
            ).all()
            if len(tabs) > 0:
...
class TestTokenUsageDateFilter:
    ...
    def test_token_usage_date_filter(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        if range_picker.count() > 0:
            ...
            if date_panel.count() > 0:
                ...
                if today_cell.count() > 0 and today_cell.is_visible(
                    timeout=2000
                ):
                    today_cell.click()
                    ...
                    if is_range:
                        ...
                        end_cells = date_panel.locator(
                            "td.qwenpaw-picker-cell-in-view, td.ant-picker-cell-in-view"
                        ).all()
                        if len(end_cells) > 1:
                            end_cells[-1].click()
                            page.wait_for_timeout(500)
...
class TestTokenUsageEmptyState:
    ...
    def test_token_usage_empty_state(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        loading = page.locator(
            '.qwenpaw-spin, .ant-spin, [class*="loading"]'
        ).first
        ...
        empty_state = page.locator(
            '.qwenpaw-empty, :text("暂无"), :text("No data"), :text("Empty")'
        ).first
        tables = page.locator(".qwenpaw-table, table").all()
        ...
        if empty_state.count() > 0 and empty_state.is_visible():
            ...
            logger.info(
                f"Data displayed: {len(tables)} tables, {len(cards)} cards"
            )
        else:
