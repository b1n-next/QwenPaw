...
class TestACPPageDisplay:
    ...
    def test_acp_page_load_and_card_list(
        ...
        try:
            ...
            if breadcrumb.is_visible(timeout=3000):
                ...
                assert (
                    "ACP" in breadcrumb_text
                ), f"Breadcrumb should contain ACP, actual: {breadcrumb_text}"
...
class TestCreateACPDrawerForm:
    ...
    def test_create_acp_drawer_form(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        try:
            ...
            assert command_input.is_visible(
                timeout=3000
            ...
            close_btn = drawer.locator(
                ".qwenpaw-drawer-close, .qwenpaw-modal-close"
            ).first
...
class TestACPToggleSwitch:
    ...
    def test_acp_toggle_switch(
        ...
        try:
            ...
            logger.info(
                f"Initial switch state: {'enabled' if initial_checked else 'disabled'}"
            )
            ...
            logger.info(
                f"Switch state after toggle: {'enabled' if new_checked else 'disabled'}"
            )
            assert (
            ...
            try:
                ...
                logger.warning(
                    f"Failed to restore original state: {restore_err}"
                )
    ...
    def test_filter_tabs_switch(
        ...
        try:
            ...
            if not builtin_tab.is_visible(timeout=5000):
                logger.info(
                    "Builtin tab not visible, skipping tab switch validation"
                )
                log_test_result(test_name, True, 0)
                return
        ...
        try:
            ...
            if agent_key_input.is_visible(timeout=3000):
                ...
                if drawer_title.is_visible(timeout=2000):
                    ...
                    logger.info(
                        f"agentKey field hidden (builtin protection), drawer title: {title_text}"
                    )
                else:
                    logger.info(
                        "agentKey field hidden (builtin ACP protection behavior)"
                    )
...
class TestCreateAndDeleteCustomACP:
    ...
    def test_create_and_delete_custom_acp(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        try:
            ...
            logger.info(
                f"Card count after creation: {new_count} (initial: {initial_count})"
            )
            ...
            if found_new:
                ...
                if target_card.is_visible(timeout=3000):
                    ...
                    edit_drawer = page.locator(
                        ".qwenpaw-drawer, .qwenpaw-modal"
                    ).first
...
class TestBuiltinACPProtection:
    ...
    def test_builtin_acp_protection(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        try:
            ...
            if len(cards) == 0:
                logger.info(
                    "No builtin ACP cards found, skipping protection validation"
                )
                log_test_result(test_name, True, 0)
                return
            ...
            if key_input.is_visible(timeout=3000):
                ...
                assert (
                    is_disabled or is_readonly
                ), "Builtin ACP agentKey should be disabled or readonly"
                logger.info(
                    "Builtin ACP agentKey is not editable (disabled/readonly)"
                )
            else:
                ...
                assert (
                    hidden_item.count() > 0 or not key_input.is_visible()
                ), "Builtin ACP agentKey should be hidden or non-editable"
                logger.info(
                    "Builtin ACP agentKey is hidden (non-editable protection in effect)"
                )
            ...
            if delete_btn.is_visible(timeout=3000):
                ...
                logger.info(
                    "Builtin ACP has no delete button (protection in effect)"
                )
...
class TestACPCardDetails:
    ...
    def test_acp_card_content_details(
        self, page: Page, request: pytest.FixtureRequest
    ):
        ...
        try:
            ...
            assert (
                cards_with_key > 0 or cards_with_switch > 0
            ), "At least some ACP cards should contain agentKey identifier or enable/disable switch"
            logger.info(
                f"Card details validation passed (with key: {cards_with_key}, with switch: {cards_with_switch})"
            )
