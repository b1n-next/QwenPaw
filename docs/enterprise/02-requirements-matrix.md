...
| A6 | 弹性扩缩容（Hub 层 HPA；runtime per-tenant 不扩副本） | HUB | 🟡（**2026-09-19 拍板：外置 Postgres**（否决单副本长期化）；28 号路线图三阶段——**P0+P1 主干已落**（docker PG17 实测）：`db_adapter.py`（双驱动/占位符/% 转义/CHECK json_valid·COLLATE NOCASE 剥除/datetime('now')·randomblob·substr 函数映射/`PgRow` 兼容 sqlite3.Row 位置+名称双索引）；`connect_hub_database` 单点切换；自省（sqlite_master↔information_schema）与 rowid↔ctid（审计链 6 处+回填）方言化；auth JSON1 改写（json_set→jsonb||build_object）；**live PG 冒烟 2 例**（HTTP 层级/D5 审批/审计链 verify 全绿，`QWENPAW_PG_TEST_URL` 驱动、无 PG 自动 skip）；SQLite 路径全量回归零变化；P2=多副本+HPA+搬迁脚本 | P2 | P0 已落 | 高 |
...
| C6 | 组织层级（租户→部门→团队四级） | GLM | ✅（groups.parent_id（幂等 ALTER+FK）；create 可挂父+**防环/自父/深于 4 级拒绝**；`descendant_member_ids` 子树成员展开（G3 组配额自动含子团队）；`policies_for` 组链继承（叶子成员继承部门 policy）；list_groups 带 parent_id；**HTTP 面补透传**（POST /admin/groups 接 parent_group_id+审计 group.created——PG 冒烟抓到的漏项）；7 测试见身份批+PG 冒烟） | P3 | 已落 | 中 |
...
| E10 | 计费精度到对话/Agent 级 | GLM | 🟡（**Agent 级 ✅**：costs 端点 `by_agent`（usage_counters PK 含 agent_id 全链路）+ **CSV 计费导出** `GET /admin/usage/costs/export`——行级=tenant×agent×provider×model，**逐行精确计价**（无混合估算），组归属导出时解析，多币种逐行保留，审计 `usage.costs.export`；**对话级 🟡→**C1-C3 已落（fork 侧）**：`_UsageEvent.session_id`（record 自动附 ambient session）+ buffer `sessions` 旁挂段（session×date×agent×provider×model，聚合计数不动=配额零影响）+ `usage_for_session()` 下钻查询；3 测试（token_usage 套件 96 绿）；**C4-C6 跨平面等上游**（issue 草稿 `upstream-issue-conversation-billing.md` 待用户过目提交）；hub 导出对话级在上游落地前如实挂起）；7 测试） | P3 | 部分已落 | 中 |
