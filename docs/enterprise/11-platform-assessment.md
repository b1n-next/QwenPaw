...
| G-P14 | 会话单机 JSON、沙箱无容器后端、无移动 H5 审批 | 规模化 | 低（Phase 3） | ✅ 三项全落：会话 SQLite（`QWENPAW_SESSION_STORE=sqlite`）；容器沙箱（`SandboxMode.CONTAINER`，gVisor/Kata 经 `--runtime`）；移动 H5 审批（`/mobile/approvals` 零依赖单页，复用 `/api/approval/*`，Bearer token + localStorage） |
