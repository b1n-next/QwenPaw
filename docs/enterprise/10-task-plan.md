...
| EP-2-6 🔶 | 问数应用 M1（封装版闭环）——代码+测试已落（`plugins/apps/qa-data/`），金集人工判待应用线接真模型 | 5d |
...
> **状态注记（2026-09-17 收口）**：**平台底座全量完善**——十票中七票交付
> （EP-2-1/2/3/4/5/9/10），另有矩阵直落缺口 12 项清零
> （H2/H3/B5/E4/E6/E7/A4/G2/E9/F5/F9 + G7 两级执行；E5 仅配置面落地、切换消费面留待应用线，矩阵保持 🟡）。
> **EP-2-6..8b（问数 M1-M4）**：M1 已解冻开工（2026-09-18，
> 除市场外全部开工指示）；M2-M4 顺序推进。
> 底座面 `enterprise/v0.3` 收口打 tag（问数面完成后再升 v0.4）。
> 另：Phase 3 无独立票据，G-P14 三项（会话后端 ×2 + 容器沙箱 + H5 审批）已提前落。
